...
app.post('/api/v1/updatelist', (req, res, next) => {
    const studentsFromCache = parseNewStudentsFromCache(req);
    console.log('POST /api/v1/updatelist', students);
    students.updateAll(studentsFromCache).then((result) => res.json(result));
});
...
function parseNewStudentsFromCache(req, res) {
    var newStudents = [];
    req.body.forEach( student => {
        newStudents.push({
            name: student.name,
            picSrc: student.picture,
            bio: student.bio,
            id: student.id
        })
    })
